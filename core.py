...
# ---------------------------------------------------------------- מסווג-תשובות משותף (זהה לסורק הארגוני)
_DONE_WORDS = ['טופל', 'טיפלתי', 'תוקן', 'תיקנתי', 'תוקנה', 'עודכן', 'עדכנתי', 'עודכנה', 'בוצע', 'ביצעתי', 'הוקם', 'הקמתי', 'הוקמה', 'שודר', 'שידרתי', 'סווג מחדש', 'סיווגתי מחדש', 'שוייך מחדש', 'שייכתי מחדש', 'הושלם', 'השלמתי', 'נסגר', 'סגרתי', 'סודר', 'סידרתי', 'הסתיים', 'סיימתי', 'נעשה', 'הסתדר', 'שונה', 'שיניתי', 'הוחלף', 'החלפתי', 'הוזן', 'הזנתי', 'נחסם', 'חסמתי', 'בוטל', 'ביטלתי', 'נמחק', 'מחקתי', 'הוסר', 'הסרתי', 'אושר', 'אישרתי', 'טופל ותוקן', 'כבר טופל', 'כבר תוקן', 'כבר עודכן', 'טופל במלואו', 'תוקן בהתאם', 'בוצע כנדרש', 'הושלם הטיפול', 'הטיפול הושלם', 'הכל טופל', 'הכל תוקן', 'גמור', 'מוכן']
_WIP_WORDS = ['עדיין', 'טרם', 'בבדיקה', 'בודק', 'בודקת', 'נמצא בבדיקה', 'ממתין', 'ממתינה', 'בהמתנה', 'בתהליך', 'בתהליך טיפול', 'בתהליך הקמה', 'בטיפול', 'נמצא בטיפול', 'יטופל', 'אטפל', 'אטפל בהקדם', 'אבדוק', 'אבדוק ואחזור', 'אעדכן', 'אעדכן בהמשך', 'אשלים', 'אשלים בהמשך', 'אחזור אליך', 'אנסה שוב', 'לא הספקתי', 'לא הספקתי לטפל', 'לא טופל', 'לא בוצע', 'לא תוקן', 'לא הוקם', 'טרם טופל', 'טרם בוצע', 'טרם הוקם', 'עדיין לא', 'עוד לא', 'בעבודה', 'נמצא בעבודה', 'בשלבי טיפול', 'לא סיימתי', 'לא הצלחתי', 'אני על זה', 'אני מטפל', 'אני בודק', 'ממתין לאישור', 'ממתין לתשובה', 'ממתין למידע', 'ממתין לספק', 'התחלתי לטפל', 'בקרוב יטופל']
_NA_WORDS = ['לא רלוונטי', 'אינו רלוונטי', 'אין ליקוי', 'אין שגיאה', 'אין טעות', 'אין כאן שגיאה', 'אין פה שגיאה', 'לא שגיאה', 'זו לא שגיאה', 'זה לא שגיאה', 'לא מדובר בשגיאה', 'לא מדובר בליקוי', 'אינו שגוי', 'אינה שגויה', 'טעות בבקרה', 'הבקרה שגויה', 'הבקרה טעתה', 'הבקרה לא נכונה', 'אין צורך בתיקון', 'לא נדרש תיקון', 'לא דורש תיקון', 'אין מה לתקן', 'לא צריך לתקן', 'אין ממצא', 'אין תקלה', 'הפריט תקין', 'המק"ט תקין', 'הכל תקין', 'הקטלוג תקין', 'השיוך תקין', 'ההיררכיה תקינה', 'הסיווג נכון', 'השיוך נכון', 'מסווג נכון', 'מקוטלג נכון', 'נכון להשאיר', 'זה נכון', 'זה תקין']
_Q_WORDS = ['?', 'מדוע', 'למה', 'כיצד', 'לא ברור', 'לא ברור לי', 'לא הבנתי', 'לא הבנתי מה', 'לא מבין', 'לא מבינה', 'מה חסר', 'מה הבעיה', 'מה השגיאה', 'מה הכוונה', 'למה הכוונה', 'מה נדרש', 'מה צריך', 'מה בדיוק', 'על מה מדובר', 'במה מדובר', 'נא לפרט', 'נא להבהיר', 'נא להסביר', 'אשמח להבהרה', 'אשמח לפירוט', 'מבקש הבהרה', 'מבקשת הבהרה', 'דרושה הבהרה', 'אפשר לפרט', 'תוכל לפרט', 'תוכלי לפרט', 'איזו שגיאה', 'איזה ליקוי']
...
def _extract_new_body(b):
    """הטקסט החדש של המגיב — חיתוך ציטוט/חתימה (זהה ל-ExtractNewBody בסורק)."""
    s = str(b or "")
    cut = len(s) + 1
...
def _has_handled_clean(t):
    for w in _DONE_WORDS:
        pos = t.find(w)
        while pos >= 0:
            if not any(neg in t[max(0, pos - 12):pos] for neg in _NEGATORS):
                return True
            pos = t.find(w, pos + 1)
    return False


def classify_reply(raw):
    """מסווג תשובת-אנליסט (מלל חופשי): DONE/WIP/NA/ASK. זהה ללוגיקת הסורק הארגוני."""
    t = " " + " ".join(str(raw or "").lower().split()) + " "
    wip = any(w in t for w in _WIP_WORDS)
    na = any(w in t for w in _NA_WORDS)
    dn = _has_handled_clean(t)
    qq = ("?" in str(raw or "")) or any(w in t for w in _Q_WORDS)
    if wip:
        return "WIP"
    if na and not dn:
        return "NA"
...
def reply_tags(text):
    """כל תגיות [#בקשה-שורה] בטקסט (ייחודי, לפי סדר הופעה)."""
    seen, out = set(), []
    for req, line in _TAG_RE.findall(str(text or "")):
        k = req + "-" + line
        if k not in seen:
            seen.add(k)
            out.append((req, line))
...
def apply_mail(led: dict, mail: dict) -> dict:
    ...
    if mail["mail_type"] == "משלוח":
        ...
        # ערוץ-גרירה משתמש באותו מסווג כמו הסורק הארגוני: תגית פר-פריט אחרת שולח+תקופה;
        # סיווג המלל -> DONE/WIP/NA/ASK עם אותן הגנות (בולק/תקופה לא סוגר; NA לא סוגר לבד).
        snds = re.findall(r"[\w.\-]+@[\w.\-]+", str(mail.get("sender", "")).lower())
        body_new = _extract_new_body(mail.get("body", ""))
            ...
            for i in f.index:
                if tags:
                    ...
                    hit = (bool(snd)
                           ...
                           and (not period or f.at[i, "תקופת בקרה"] == period))
                if not hit or f.at[i, "סטטוס"] != "נשלח":
                    continue
                d = "ASK" if (multi and decision == "DONE") else decision
                pre = ("⚠ דרוש אימות (האנליסט טוען: לא רלוונטי) · " if d == "NA"
                       else ("⚠ דרוש אימות · " if d == "ASK" else ""))
                f.at[i, "סטטוס"] = _map[d]
                f.at[i, "חיווי בתאריך"] = when
                f.at[i, "מקור חיווי"] = "מייל"
                if body_new:
                    cur = str(f.at[i, "הערה"] or "")
                    f.at[i, "הערה"] = (cur + " | " if cur else "") + pre + "תשובה: " + body_new[:120]
                f.at[i, "עדכון אחרון"] = now_str()
                touched += 1
    led["findings"] = f
    s = led["shipments"]
